...
> **Dernière mise à jour:** Mai 2026 (clustering map, additional fees PostulerDrawer, photo upload HEIC, PDF viewers, tutorial overlay)

---

## Composants ajoutes / mis a jour depuis decembre 2025

| Composant | Fichier | Changement notable |
|-----------|---------|-------------------|
| **PostulerDrawer** | `components/shared/PostulerDrawer.vue` | Refonte complete : 7 etapes pour mission payante (dispo, materiel, prep, TJM avec clamp + billing_days override, additional fees, CGV, recap avec preview devis) ; validation des `additional_fee_lines` (max 20, 160 chars, 2 decimales) via `utils/additional-fees.js` ; integration `buildGotchaCommissionAmounts` (13% HT + TVA) |
| **Map (shared/map)** | `components/shared/map/Map.vue` | Composant Mapbox de base (zone d'intervention cercle + radar anime pulse 3s, missions markers). Le **clustering des providers est implemente dans `pages/map/search-provider.vue:505+`** (custom pixel-based, `CLUSTER_PX_THRESHOLD = 50`, PAS Mapbox native), avec markers DOM photo stackees + badge count. Badge "Disponible maintenant" sur `ProviderCard` |
| **ContractRecap (entreprise)** | `components/entreprise/contrat/ContractRecap.vue` | Affichage des `additional_fee_lines` (icone lucide:plus-circle), TVA 20% conditionnelle sur `provider.regime_tva`, timeline paiement escrow (pre-auth + capture finale) |
| **ContractRecapProvider** | `components/candidat/contrat/ContractRecapProvider.vue` | Recap cote prestataire avec heures, preparation, frais additionnels |
| **ContractSignatureProvider** | `components/candidat/contrat/ContractSignatureProvider.vue` | PDF scroll detection (reachedBottom) + ConsentCheckbox versionne (`CONTRACT_SIGNATURE_CONSENT_VERSION_ID`) + `OtpVerificationModal` + wording "IP chiffree en base" (commit `ac6ebba`) |
| **DevisPdfViewer / ContractPdfViewer / InvoicePdfViewer / PdfJsScrollViewer** | `components/shared/` | Viewers PDF avec detection scroll-end (callback `onReachedBottom`) — bug PDF viewer fixe (`fbbcde8`) |
| **CgvSelector** | `components/shared/CgvSelector.vue` | Affichage CGV courantes (custom ou defaut) + upload PDF dans le PostulerDrawer |
| **PhotoUploadSection** + `SelectFile` | `components/shared/UI/SelectFile.vue` | Upload photo profil (commit `15a643b`) : compression client maxWidth=1200 / quality=0.8, support **HEIC/HEIF iPhone** (`computedAcceptTypes`), preview via `getFile` |
| **TutorialOverlay / TutorialSpotlight / TutorialTooltip / TutorialArrow** | `components/shared/Tutorial*.vue` | Onboarding interactif declenche apres `done_onboarding` si `tutorial_completed = false` (cf. `add_tutorial_completed.sql` + `composables/useTutorial.js`) |
| **GeolocationUpdateDrawer** | `components/shared/GeolocationUpdateDrawer.vue` | Verification que le prestataire est dans sa zone d'intervention (geo check au demarrage de l'app via `plugins/geolocation-check.client.js`) |
| **PwaInstallPopup** | `components/shared/PwaInstallPopup.vue` | Prompt installation PWA |
| **ProviderReportRecapDrawer** | `components/candidat/` | Recap du rapport prestataire avec apercu de la facture (`/api/invoices/preview`) avant submission |
| **CancelMissionDrawer / CancelMissionModal** | `components/candidat/`, `components/entreprise/` | Annulation v2 document-aware (snapshot contrat + payment_flow dans `providers.cancellation_logs`) |
| **OpenDisputeDrawer** | `components/entreprise/OpenDisputeDrawer.vue` | Formulaire de signalement (motifs : hours_disagreement, payment_issue, quality_issue, no_show, other) — `missions.disputes` |
| **ReplacementProvidersDrawer** | `components/entreprise/ReplacementProvidersDrawer.vue` | Recherche de remplacement apres annulation prestataire |
| **DesktopSidebar / DesktopSidePanel** | `components/shared/` | Layout desktop (cf. `layouts/default.vue`) |
| **ConfirmModal / SuccessPopup / BasePopup** | `components/shared/` | Popups generiques pour cascade de notifications (DevisAcceptedPopup, ContractFinalizedPopup, MissionFinishedPopup, etc.) |
| **MessageBubble / MessageInput / DateSeparator / SystemMessage** | `components/shared/messaging/` | Chat chiffre (`useMessaging`, `send_message_v3` RPC) |
| **Switch (custom) / SwitchInput** | `components/shared/SwitchInput.vue` | Toggle 2 options |
| **InputCalendar / RangeCalendar / DateInput / TimeInput** | `components/shared/UI/` | Inputs date/heure locale FR ; `RangeCalendar` pour missions multi-jours |
| **ImportNumber** | `components/shared/UI/ImportNumber.vue` | Input numerique avec +/-, suggestions, `reference-price`, `floor-multiplier`, clamp min/max (utilise pour le TJM dans PostulerDrawer) |

## Cascade de popups (layouts/default.vue)

Les popups sont affichees par priorite (un seul a la fois) :
- **P0** : `DevisAcceptedPopup` (provider, broadcast realtime quand entreprise signe le devis)
- **P1** : `MissionSavedPopup` (company), `ContractPendingPopup` (provider)
- **P1.5** : `ContractSignPendingPopup` (provider)
- **P2** : `ContractFinalizedPopup` (company, nouveau flow apres signature provider), `PendingReportPopup` (provider)
- **P3** : `MissionFinishedPopup` (provider/company, declenche rating drawer)
- Plus bas : `PwaInstallPopup`
- Plus haut Z-index : `TutorialOverlay`
